module.exports = {
    validatePosts(req, res, next) {
        if(req.method == "POST") {
            ...
            req.checkBody("title", "must be at lease 2 characters in length").isLength({min: 2});
    ...
    validateTopics(req, res, next) {
        if(req.method == "POST") {
            ...
            req.checkBody("description", "must be at least 10 charecters in length").isLength({min: 10});
